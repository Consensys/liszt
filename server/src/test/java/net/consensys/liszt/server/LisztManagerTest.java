...
public class LisztManagerTest {
  ...
  public void validTransfersShouldUpdateAccountBalance() throws Exception {
    for (int i = 0; i < 10; i++) {
      boolean isValid =
          lisztManager.addTransfer(TestUtils.createMockTransferFromAliceToBob(i, BigInteger.valueOf(5)));
  ...
  public void illegalTransfersShouldBeRejected() {
    for (int i = 0; i < 10; i++) {
      boolean isValid =
          lisztManager.addTransfer(TestUtils.createMockTransferFromAliceToBob(i, BigInteger.valueOf(800)));
      Assert.assertTrue(isValid);
    }
    Account aliceAcc = lisztManager.getAccount(TestUtils.alice);
  ...
  public void illegalTransfersShouldBeFilteredOut() {
    for (int i = 0; i < 21; i = i + 2) {
      lisztManager.addTransfer(TestUtils.createMockTransferFromAliceToBob(i, BigInteger.valueOf(800)));
      lisztManager.addTransfer(TestUtils.createMockTransferFromAliceToBob(i + 1, BigInteger.valueOf(5)));
    }
    Account aliceAcc = lisztManager.getAccount(TestUtils.alice);
    Assert.assertEquals(aliceAcc.amount, BigInteger.valueOf(55));
  }
  ...
  public void mixTransfers() throws Exception {
    for (int i = 0; i < 10; i++) {
      boolean isValid =
          lisztManager.addTransfer(TestUtils.createMockXTransferFromAliceToBob(i, BigInteger.valueOf(5)));
      Assert.assertTrue(isValid);
      isValid =
          lisztManager.addTransfer(TestUtils.createMockTransferFromAliceToBob(i, BigInteger.valueOf(5)));
      Assert.assertTrue(isValid);
